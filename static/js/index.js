...
function validate_dataset_upload(){
	var fInput = document.getElementById("dataset")
	var flag = 0
	if(fInput.value.length!=0){
		/* file validation */
		var validExt= /(\.csv|\.xslx)$/i;
		if (!validExt.exec(fInput.value)) {
			document.getElementById("filecheck").innerHTML='<font color="red">Only CSV and EXCEL formats are supported!</font>';
fInput.style.borderColor="red";
			flag=0;
		}
		else{
